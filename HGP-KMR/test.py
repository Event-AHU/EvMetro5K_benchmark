...
def main():
    ...
    test_set = SceneTextDataModule.TEST_SUBWAY
    if args.new:
    ...
    # for name, dataloader in datamodule.test_dataloaders(test_set):
    #     total = 0
    #     correct = 0
    #     ned = 0
    #     confidence = 0
    ...
    for event_modality, rgb_modality in datamodule.test_dataloaders(test_set):
        res = model.test_step((event_modality, rgb_modality), -1)['output']
        total += res.num_samples
        correct += res.correct
        ned += res.ned
        confidence += res.confidence
    ...
    # }
    result_groups = {
        'Subway_dataset': [SceneTextDataModule.TEST_SUBWAY],
    }
    if args.new:
