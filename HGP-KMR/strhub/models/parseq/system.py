...
class PARSeq(CrossEntropySystem):
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        # images, labels = batch
        event_images, labels = batch[0]
        rgb_images, labels = batch[1]
        ...
        event_emb = self.model.patch_embedding(event_images)
        rgb_emb = self.model.patch_embedding(rgb_images)

        event_memory = self.model.encode(event_emb) # torch.Size([64, 3, 32, 128]) → torch.Size([64, 128, 384])
        # rgb_memory = self.model.encode(rgb_emb) # torch.Size([64, 3, 32, 128]) → torch.Size([64, 128, 384])
        
        rgbe_memory = torch.cat((event_memory, rgb_emb), dim=-1)
        _, G = self.model.build_G(rgbe_memory)
        hg_memory = self.model.hypergraph_encoder(rgbe_memory, G)

        memory = self.model.encode(rgb_emb, hg_memory)
