...
#         x = self.dropout(x_norm) + x
#         return x

# class HypergraphAttentionEncoder(nn.Module):
#     def __init__(self, in_dim, hidden_dim, out_dim, dropout=0.1):
#         super().__init__()
#         self.dropout = nn.Dropout(dropout)
        
#         # 超图卷积
#         self.hgconv1 = HypergraphConv(in_channels=in_dim, out_channels=hidden_dim)
#         self.hgconv2 = HypergraphConv(in_channels=hidden_dim, out_channels=out_dim)
        
#         # 注意力层
#         self.hgattn1 = HypergraphAttentionLayer(hidden_dim, dropout=dropout)
#         self.hgattn2 = HypergraphAttentionLayer(out_dim, dropout=dropout)
...
#         """
#         B, N, _ = X.shape
#         X_flat = X.reshape(B * N, -1)  # 展开 batch 节点
...
#         # hgconv1
#         x = self.hgconv1(X_flat, edge_index, edge_weight)
#         x = F.relu(x)
#         x = self.norm1(x)
#         x = self.dropout(x) + X_flat
...
#         # hgconv2
#         x2 = x.reshape(B * N, -1)
#         x = self.hgconv2(x2, edge_index, edge_weight)
#         x = F.relu(x)
#         x = self.norm2(x)
#         x = self.dropout(x) + x2
...
#     tokens = torch.randn(B, N, d_in, device=device)

#     # 构建超图 G
#     _, G = build_H_and_G_from_tokens(tokens, K_list=(8, 16), is_probH=True, m_prob=1.0)

#     encoder = HypergraphAttentionEncoder(d_in, hidden, d_out, dropout=0.1).to(device)
#     tokens_h = encoder(tokens, G)

#     print("输入 tokens 形状:", tuple(tokens.shape))
#     print("编码后 tokens 形状:", tuple(tokens_h.shape))